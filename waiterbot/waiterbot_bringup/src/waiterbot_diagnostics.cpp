...
#include <waiterbot_msgs/SensorState.h>
#include <waiterbot_msgs/VersionInfo.h>
#include <string>
...
void sensorStateMsgCallback(const waiterbot_msgs::SensorState::ConstPtr &msg)
{
  if (msg->battery > 11.0)
  ...
  if (msg->button == waiterbot_msgs::SensorState::BUTTON0)
    setButtonDiagnosis(diagnostic_msgs::DiagnosticStatus::OK, "BUTTON 0 IS PUSHED");
  else if (msg->button == waiterbot_msgs::SensorState::BUTTON1)
...
void firmwareVersionMsgCallback(const waiterbot_msgs::VersionInfo::ConstPtr &msg)
{
  ...
  if (check_version == false)
  {
    if (firmware_version.major_number == FIRMWARE_VERSION_MAJOR_NUMBER)
    {
      if (firmware_version.minor_number > FIRMWARE_VERSION_MINOR_NUMBER)
      {
        ...
        ROS_WARN("You can find how to update its in `FAQ` section(waiterbot.robotis.com)");
      }
    }
    else
    {
  ...
  waiterbot_msgs::VersionInfo version;
...
{
  ros::init(argc, argv, "waiterbot_diagnostic");
  ros::NodeHandle nh;

  tb3_diagnostics_pub  = nh.advertise<diagnostic_msgs::DiagnosticArray>("diagnostics", 10);
  tb3_version_info_pub = nh.advertise<waiterbot_msgs::VersionInfo>("version_info", 10);
